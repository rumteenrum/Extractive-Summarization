...
def extract_summary_with_rerank(article, pred_labels, pred_dist, opts, vocab, nsent_budget, nword_budget, trigram_block):
...
def generate_summary(article, summary, true_labels, pred_labels, pred_dist, opts, vocab, nsent_budget, nword_budget, trigram_block):
    if opts['eval_type'] == 'lead':
        ...
        if not opts['rerank']:
            ...
            return extract_summary_with_rerank(article, pred_labels, pred_dist, opts, vocab, nsent_budget, nword_budget, trigram_block)
    else:
...
def evaluate_extractive(result_file, article_file, summary_file,
    ...
    outdir = '__tmp__rouge.%d' % os.getpid()
    mkdir(outdir)
    ...
    try:
        for docid in range(ndoc):
            ...
            pred_summary = generate_summary(article, summary, true_labels,
                           pred_labels, pred_dist, opts, vocab, nsent_budget, nword_budget, trigram_block)
